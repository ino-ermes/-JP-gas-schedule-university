...
public class AdminFormMakeSchedule extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        btnGASchedule.setText("スケジュールを立てる");
    ...
    private void btnGAScheduleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGAScheduleActionPerformed
        if(courseClasses.isEmpty()) {
            new MessageDialog(Main.getInstance(), "スケジュールするクラスはありません！", false).showDialog();
            return;
        }
    ...
    private void btnCheckActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCheckActionPerformed
        ...
        if(isScheduled) {
            ...
            new MessageDialog(Main.getInstance(), "まだ全てのクラスはスケジュールされていない！", false).showDialog();
        }
    ...
    private void btnApplyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnApplyActionPerformed
        ...
        if(isScheduled) {
            if(new MessageDialog(Main.getInstance(), "注意：「適用」する前に「チェック」をクリックして、時間割の適合性を確認してください」\n続行しますか？", true).showDialog()) {
                for(CourseClass cc: courseClasses) {
    ...
    private void btnDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDelActionPerformed
        int r = table.getSelectedRow();
        if(r != -1) {
            if(new MessageDialog(Main.getInstance(), "このクラスを削除しますか？", true).showDialog()) {
            ...
            case NEW_SECTION -> {
                ...
                switch(fillStatus) {
                    ...
                    case AFTER -> {
                        btnDel.setEnabled(false);
                        btnAdd.setEnabled(false);
                        btnEdit.setText("割り当て");
                        btnFillClass.setText("確認");
    ...
    private final class MyModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            switch (columnIndex) {
                ...
                case 5 -> {
                    return row.getTeacherId() == -1 ? "未定" : teachers.get(row.getTeacherId()).getName();
                }
                default -> {
                    if(row.getRoomId() == -1)
                        return "";
                    ...
                    return "" + rooms.get(row.getRoomId()).getName() + " " + days[day] + " " + (time + 1) + "-" + (time + dur);
                }
            }
        }
